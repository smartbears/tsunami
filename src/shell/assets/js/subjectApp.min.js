...
SubjectApp.MedicationRoute = Ember.Route.extend({
    model: function(a) {
        return this.store.find("medication", a.medication_id);
    }
});
...
SubjectApp.Subject = DS.Model.extend({
    ...
    fullName: function() {
        ...
        if (b != null && a != null && c != null) return b + " " + a + " " + c; else if (b == null && a != null && c != null) return "NoFirstName " + a + " " + c; else if (b != null && a == null && c != null) return b + " NoMiddleName " + c; else if (b != null && a != null && c == null) return b + " " + a + " NoLastName"; else return "NoFirstName NoMiddleName NoLastName";
    }.property("firstName", "middleName", "lastName"),
    age: function() {
        var a = Date.now() - this.get("birthday").getTime();
        var b = new Date(a);
...
SubjectApp.SelectView = Ember.Select.extend({
    change: function() {
        var a = this;
        this._super();
        var b = this.get("onChange");
        if (b) {
            Em.run.later(function() {
                a.get("controller").send(b);
            });
        }
    }
});
...
SubjectApp.AllergiesAddController = Ember.ObjectController.extend({
    actions: {
        add: function() {
            var a = this.store.createRecord("allergy", {
                name: this.get("name"),
...
SubjectApp.SubjectsAddController = Ember.ObjectController.extend({
    ...
    actions: {
        ...
        nextInfo: function() {
            ...
            } else if (this.get("inInmunization")) {} else {}
        },
        saveModel: function() {
            var a = this.get("model");
            var b = this;
            if (a.validateSubject()) {
                a.save().then(function() {
                    var c = a.get("medications");
        ...
        addNewMedication: function() {
            var a = this.get("model");
            var b = this.store.createRecord("medication", {
                name: "Medication Name"
            });
            a.get("medications").addObject(b);
        }
    },
    hideAll: function() {
    ...
    updateBirthday: function() {
        var a = this.get("birthdayYear");
        var b = this.get("birthdayMonth");
        var c = this.get("birthdayDay");
        this.set("birthday", new Date(a, b, c));
    }
});
