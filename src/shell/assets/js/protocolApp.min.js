...
Handlebars.registerHelper("ifIsNthItem", function(a) {
    var b = a.data.view.contentIndex + 1, c = a.hash.nth;
    if (b % c === 0) {
        return a.fn(this);
    }
});
...
App.DragElementComponent = Ember.Component.extend({
    ...
    dragStart: function(a) {
        if (this.get("elementName")) {
            var b = {};
            b.container = true;
            b.item = this.get("item");
            b.elementName = this.get("elementName");
...
ProtocolApp.Protocol.reopenClass({
    FIXTURES: [ {
        id: 1,
        name: "L1V-MC-4590MK",
        description: "Diabetes Melitus II con uso de Meenformin 500g.",
...
ProtocolApp.ProcedureController = Ember.ObjectController.extend({
    ...
    actions: {
        editName: function() {
            if (this.get("name").toLowerCase() == "new procedure") this.set("name", "");
            this.set("isEditingName", true);
        },
        updateName: function() {
            if (this.get("name") == "") this.set("name", "New Procedure");
...
ProtocolApp.ProtocolsConfigureController = Ember.ObjectController.extend({
    ...
    visitCount: function() {
        var a = this.get("visits");
        return a.get("length");
    }.property("visits.@each"),
    proceduresCount: function() {
        var a = this.get("procedures");
        return a.get("length");
    ...
    actions: {
        ...
        add_procedure: function() {
            ...
            var b = this.get("procedures");
            a.save().then(function() {});
        },
        rename_procedure: function(a) {
            a.set("isEditing", true);
        },
        add_visit: function() {
            var a = "Visit " + this.get("visitCount");
            var b = this.store.createRecord("visit", {
                label: a,
                number: this.get("visitCount") + 1
            });
            this.get("visits").pushObject(b);
        },
        add: function() {
            var a = this.get("model");
            a.save().then(function() {
...
ProtocolApp.VisitController = Ember.ObjectController.extend({
    ...
    actions: {
        drop_procedure_into_visit: function(a) {
            var b = this.get("controllers.protocolsConfigure").get("procedures").findBy("id", a);
            var c = parseInt(b.id);
            if (!this.model.get("procedureIds")) this.model.set("procedureIds", [ c ]); else if (!this.model.get("procedureIds").contains()) this.model.get("procedureIds").pushObject(c);
        },
        insertNewProcedure: function(a) {
            var b = this.store.createRecord("procedure", {
                name: "New Procedure"
            });
            a.get("procedures").pushObject(b);
            if (!this.model.get("procedureIds")) this.model.set("procedureIds", [ b.id ]); else this.model.get("procedureIds").pushObject(b.id);
        },
        editLabel: function() {
    ...
    procedures: function() {
        var a = this.model.get("procedureIds");
        if (!a) return [];
        return this.get("controllers.protocolsConfigure").get("procedures").filter(function(b) {
            return a.contains(parseInt(b.id));
        });
    }.property("model.procedureIds.@each"),
    proceduresCount: function() {
        var a = this.get("procedures");
        if (!a) return 0; else return a.get("length");
    }.property("procedures.@each"),
