App = Em.Application.create({
...
}), App.MongoSample = Ember.Object.extend(), App.MongoSample.reopenClass({
    all: function() {
        return $.getJSON("http://localhost:8081/api/subjects").then(function(a) {
            var b = [];
            return a.forEach(function(a) {
                b.push(App.MongoSample.create(a));
            }), b;
        });
    },
    one: function() {
        return App.MongoSample.create({
            name: "no-name",
            age: 0
        });
    },
    save: function(a) {
        console.log("calling save method..."), $.post("http://localhost:8081/api/subjects/createsubject", {
            Name: a.name,
            Age: a.age
        }, function(a) {
            alert("Data Loaded: " + a);
    ...
    actions: {
        removeSubject: function() {
            ...
            $.ajax({
                url: "http://localhost:8081/api/subjects/" + a.id,
                type: "DELETE",
                success: function(b) {
                    $("#" + a.id).remove(), alert("Result: " + b);
                }
            });
        }
    }
