...
DS.WebAPISerializer = DS.RESTSerializer.extend({
    ...
    dragStart: function(a) {
        ...
        if ("visits" == b) {
            ...
            c.contents().filter(".list-group-item").css(e), c.contents().filter(".list-group-item").contents().filter("button").remove(), 
            c.css(d), a.dataTransfer.effectAllowed = "copy", document.body.appendChild(c[0]), 
            a.dataTransfer.setDragImage(c[0], 0, 0);
        }
        if (this.get("elementName")) {
            var f = {};
            f.container = !0, f.item = this.get("item"), f.elementName = this.get("elementName"), 
            a.dataTransfer.setData("text", JSON.stringify(f));
    ...
    attributeBindings: [ "elementName", "id", "name", "acceptAction" ]
